...
async function main() {
  ...
  const customer = await c.mutation(
    api.commercial.saveCustomer,
    {
      version: 0,
  ...
  const agreement = await c.mutation(
  ...
  await c.mutation(
    api.commercial.agreementAction,
    {
      id: agreement,
      version: 1,
      action: "send",
  ...
  await check(
    ...
    async () => {
      const x = await c.mutation(
        api.commercial.createExtension,
        {
          agreement_id: agreement,
      ...
      await c.mutation(
        api.commercial.extensionAction,
        {
          id: x,
          version: 1,
          accept: true,
      ...
      await issue(
        await c.mutation(api.commercial.sourceInvoice, args, {
          skipQueue: true,
        }),
      );
      await assert.rejects(
        c.mutation(api.commercial.sourceInvoice, args, { skipQueue: true }),
      );
    },
  );
  await check(
  ...
  for (const a of assets) {
    ...
    ids.push(
      await c.mutation(
        api.commercial.createAssessment,
        {
          project_id: project,
  ...
  const review = async (id: Id<"damage_charge_assessments">) =>
    c.mutation(
      api.commercial.reviewAssessment,
      {
        id,
  ...
  const approve = async (id: Id<"damage_charge_assessments">) =>
    c.mutation(
      api.commercial.decideAssessment,
      {
        id,
        version: (await assess(id)).version,
        decision: "approve",
  ...
  await check(
    ...
    async () => {
      assert.equal((await get()).status, "destaging");
      for (const a of assets) {

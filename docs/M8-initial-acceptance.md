> Historical snapshot, superseded by [M8-report.md](M8-report.md). Statements of pending billing/audit below describe the earlier iteration only.
...
## Completed final decision criteria, sections 64–68

These criteria govern the final decision and supersede the truncated section 64. The decision remains **M8 DEVELOPMENT GATE PENDING EXTERNAL ACTION**. Partial/local evidence is not promoted to a hosted or live PASS. No additional provider request was made when applying this continuation because the quota/billing blocker has not been reported resolved. No deployment or business mutation was needed for this report-only update.

**Tested implementation commit:** `c358fa7a79feb834c39fe236ffe2da0d1aec504f`, pushed to `daryan-kf/glara-os-codex/main`. This continuation changes only acceptance documentation. The exact final report commit is returned in the delivery message and resolves with `git log -1 --format=%H -- docs/M8-report.md`; embedding its own hash in the same commit would change that hash. Deployment tested: **woozy-jaguar-392**, **development**, project **glara-os**. Provider/model: **OpenAI / gpt-5.4-mini**.

| Criterion                                | Status                                             | Evidence and remaining requirement                                                                                                                                                                                               |
| ---------------------------------------- | -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A. Live provider                         | PENDING EXTERNAL ACTION                            | Model/configuration verified. Three real adapter attempts, zero successful answers; HTTP 429 quota_or_billing. Successful structured output and actual token usage are unverified.                                               |
| B. Fictional-data isolation              | VERIFIED FOR ATTEMPTED SCOPE                       | Three new fictional Realtors only, with selected-record zero-history metrics; no company aggregate or mixed dataset sent. Full live matrix must repeat context inspection.                                                       |
| C. Authorization                         | PARTIAL                                            | Ten local authorization cases passed. Hosted disabled-provider checks cover six roles and conversation isolation, but are not the full hosted context/anonymous/archived/revoked/unassigned matrix. Full criterion remains open. |
| D. Numerical grounding                   | PARTIAL                                            | Thirteen exact local source/context comparisons are documented below. No accepted real-model numerical answer was evaluated.                                                                                                     |
| E. Evidence grounding                    | PARTIAL                                            | Local validation rejects unknown citations and unsupported numerical output; disabled navigation evidence was exercised. Live semantic support of claims remains unverified.                                                     |
| F. Hallucination / insufficient evidence | PENDING EXTERNAL ACTION                            | Six local controlled-response evaluation contracts passed; zero completed live factuality evaluations. Missing historical fact and every required live case still need explicit evidence.                                        |
| G. Prompt injection / red team           | PARTIAL                                            | Local direct/indirect injection and permission checks passed within the 68 M8 tests. The complete named hosted/live red-team matrix has not passed.                                                                              |
| H. Human approval                        | PARTIAL                                            | Local stale/expired/edited/rejected/concurrent approval and audit tests passed. Live proposal generation and visible desktop/mobile approval remain unexecuted.                                                                  |
| I. Action allowlist                      | PARTIAL                                            | Executable action is create_activity through existing CRM/sales/project mutations; drafts do not write. Local rejection checks passed. Explicit hosted denial evidence for every listed forbidden operation remains required.    |
| J. M3–M7 consistency                     | PARTIAL                                            | Local adapters use authoritative source calculations and suppress existing follow-ups; prior-module hosted regressions passed. Live answers and live approved-task coordination are unverified.                                  |
| K. Provider failure safety               | PARTIAL                                            | Three actual quota failures returned safe errors with no AI-created business mutation. Local timeout/malformed/evidence/action failures passed. Complete controlled hosted failure matrix remains open.                          |
| L. Cost / rate safety                    | PARTIAL                                            | Local concurrent budget/rate/output controls passed. Failed hosted requests recorded latency and conservative reservation charges; actual provider usage, billing and successful live accounting remain unverified.              |
| M. Conversation security                 | PARTIAL                                            | Local revocation/retention/delete/audit tests and hosted six-role ownership checks passed. Full hosted current-context/revocation/evidence matrix remains open.                                                                  |
| N. Desktop acceptance                    | PARTIAL                                            | Six disabled-AI M8 plus five authentication scenarios passed. Full enabled Analyst/record/draft/proposal/settings/error workflows remain open.                                                                                   |
| O. Mobile acceptance                     | PARTIAL                                            | Six disabled-AI M8 plus five authentication scenarios passed; tested overflow checks passed. Enabled drafts, briefs and approval remain open.                                                                                    |
| P. Fresh M1–M7 hosted regression         | PASSED FOR RECORDED ITERATION; FINAL SEQUENCE OPEN | 253 distinct checks passed during M8 acceptance. Only final M6, M7 clean smoke and M8 disabled checks were repeated after helper removal; repeat all required suites on the final clean source after remaining live acceptance.  |
| Q. Final M6 reconciliation               | PASSED FOR RECORDED ITERATION                      | After fixture writes stopped: 3,480 sources, zero source drift, zero bucket drift, revision 3066. Repeat after any future live approval/fixture mutation.                                                                        |
| R. M4/M5 integrity                       | VERIFIED FOR ATTEMPTED SCOPE                       | No accepted AI proposal/action occurred; financial/inventory fixtures were ordinary authorized non-AI setup. M4/M5 regression passed; 65 serialized assets had zero mismatches. Repeat after live action acceptance.             |
| S. Clean deployment                      | PARTIAL                                            | All three temporary modules removed and absence verified; clean M8 49-check suite and M7 smoke passed. Successful clean live/approval smoke is still pending.                                                                    |
| T. Local quality gates                   | PARTIAL                                            | 13 Node + 347 Vitest pass; TypeScript/lint/format/build/secret/whitespace checks passed. Dependency audit remains blocked and is not passed.                                                                                     |
| U. No unresolved P0/P1                   | NO KNOWN OPEN DEFECT; NOT FULL CLEARANCE           | Previously discovered rollout and validation gaps were corrected with coverage. No unresolved P0/P1 observed in completed checks; unexecuted gates cannot establish absence of defects.                                          |
| V. Safe final development state          | VERIFIED                                           | Provider configured on development only; AI and proposals disabled, no company-wide enablement, no external communications, production untouched, M9 not started.                                                                |

The 253 hosted checks are fresh evidence from this M8 attempt, not inherited milestone claims. Their sequence does not satisfy a claim that every suite was repeated after the last clean redeployment. The remaining acceptance run must finish with the full required clean-source regression, independent reconciliation and critical security smoke.

Dependency audit remains unrun following automatic review rejection of dependency metadata transfer to `registry.npmjs.org`; the specific requested approval has not arrived. Account privacy/retention/sharing/residency controls remain unverified. Resolve these external dependencies alongside OpenAI quota/billing, then resume the already authorized development-only acceptance. No real data may be used to make the evaluation pass.

P0 authorization, sensitive-data exposure or unauthorized mutation and P1 factual, numeric, evidence, human-approval or business-integrity defects require a FAILED decision until corrected. The current PENDING decision is based on documented external blockers and incomplete evaluation, not a waiver of any criterion. No new implementation defect was discovered by this documentation review. Production email/auth and all previously listed go-live dependencies remain **DEFERRED — REQUIRED BEFORE PRODUCTION**.

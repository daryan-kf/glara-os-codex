...
- Persian language and right-to-left content, with the actual English UI labels retained in the instructions.
- Local search across all chapters, supporting Persian/Arabic variants of yeh and kaf, whitespace and half-space, and case-insensitive English terms. Search text is not sent to a provider.
...
- A scoped system font stack for Persian readability, responsive layout, native keyboard-operable controls, labelled search, and live result count.
...
Verification for this change: **passed**. The final isolated run completed **22 browser scenarios** (11 desktop and 11 mobile); central authorization completed **4 tests**. Build, typecheck, lint, formatting and secret scan passed. Desktop/mobile screenshots were visually reviewed.
...
- Isolated desktop (1280 × 900) and mobile (393 × 851) browser checks using fictional local accounts: menu navigation, RTL, no horizontal overflow, Persian/Arabic and English search, empty results and reset, contents links, role-aware links, print visibility, and unauthenticated rejection. Existing login, logout, and recovery checks run alongside them.
